...
class Threads(commands.Cog):
    ...
    async def create(self, ctx: commands.Context, name: str, channel: nextcord.channel.TextChannel = None, is_public: bool = True, duration: int = 1440):
        ...
        if is_public:
            ...
            if ctx.guild.premium_tier < 2:
                await ctx.send("Private threads are only available for Tier 2 servers. Defaulting to public thread.")
                thread_type = nextcord.ChannelType.public_thread
        

        thread = await channel.create_thread(name=name, auto_archive_duration=duration, type=thread_type)
        logger.info(f"Created thread {name} (id: {thread.id}) under channel {channel.name} (id: {channel.id})")
    ...
    async def delete(self, ctx: commands.Context, thread: nextcord.Thread):
        ...
        await thread.delete()
        logger.info(f"Deleted thread {thread.name} under channel {thread.parent.name} (id: {thread.parent.id})")
    ...
    async def archive(self, ctx: commands.Context, thread: nextcord.Thread):
        ...
        logger.info(f"Archived thread {thread.name} under channel {thread.parent.name} (id: {thread.parent.id})")
    ...
    async def unarchive(self, ctx: commands.Context, thread: nextcord.Thread):
        ...
        logger.info(f"Unarchived thread {thread.name} under channel {thread.parent.name} (id: {thread.parent.id})")
